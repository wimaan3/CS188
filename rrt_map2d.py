...
# 1) load your top-down map (True=obstacle)
map_img = cv2.imread('pgm/map2d-2.pgm', cv2.IMREAD_GRAYSCALE)
# or pgm/map.pgm or pgm/map3d.pgm or pgm/map3d-2.pgm
grid2d  = (map_img == 0)
H, W    = grid2d.shape

# 2) click start and goal on the 2D view
fig, ax = plt.subplots()
ax.imshow(~grid2d, cmap='gray')
ax.set_title("Click START then GOAL")
pts = plt.ginput(2, timeout=0)
plt.close(fig)
start2d = (int(pts[0][0]), int(pts[0][1]))  # (x,z)
goal2d  = (int(pts[1][0]), int(pts[1][1]))

# 3) RRT structures & helpers
class Node:
...
# 4) build RRT until goal is reached
for i in range(max_iters):
    rnd     = (random.uniform(0, W-1), random.uniform(0, H-1))
    nearest = min(nodes, key=lambda n: math.hypot(n.x-rnd[0], n.z-rnd[1]))
    new_pt2 = steer(nearest, rnd, step)
    new_int = (int(new_pt2[0]), int(new_pt2[1]))
...
# 5) animate in 3D (X–Y–Z), with Y=0 plane
fig3d = plt.figure()
ax3d = fig3d.add_subplot(111, projection='3d')
ax3d.set_xlim(0, W)
ax3d.set_ylim(0, 1)   # flat plane at y=0
ax3d.set_zlim(0, H)
ax3d.set_xlabel('X')
ax3d.set_ylabel('Y')
ax3d.set_zlabel('Z')
...
def init():
    for ln in edge_lines: ln.set_data([], []); ln.set_3d_properties([])
    path_line.set_data([], []); path_line.set_3d_properties([])
    robot_dot.set_data([], []); robot_dot.set_3d_properties([])
...
def animate(i):
    if i < len(edges):
        ...
        edge_lines[i].set_data(xs, ys); edge_lines[i].set_3d_properties(zs)
        return [edge_lines[i]]
    else:
        ...
        path_line.set_data(xs, ys); path_line.set_3d_properties(zs)
        robot_dot.set_data(xs[-1:], ys[-1:]); robot_dot.set_3d_properties(zs[-1:])
